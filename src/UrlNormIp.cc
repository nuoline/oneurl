...
template<typename CHAR, typename UCHAR>
bool DoNormalizeIPv4Address(const CHAR* spec,
                               const Component& host,
                               CanonOutput* output,
    ...
    if (is_colon) {
      ...
      if (!IsHexChar(static_cast<unsigned char>(spec[i]))) {
        ...
        if (IsIPv4Char(static_cast<unsigned char>(spec[i]))) {
          ...
          // later).
          parsed->ipv4_component = Component(
...
// is NEUTRAL, and we could use a second opinion.
template<typename CHAR, typename UCHAR>
bool DoNormalizeIPv6Address(const CHAR* spec,
                               const Component& host,
                               CanonOutput* output,
                               CanonHostInfo* host_info) {
...
void NormalizeIPAddress(const char* spec,
                           const Component& host,
                           CanonOutput* output,
                           CanonHostInfo* host_info) {
  if (DoNormalizeIPv4Address<char, unsigned char>(
          spec, host, output, host_info))
    return;
  if (DoNormalizeIPv6Address<char, unsigned char>(
          spec, host, output, host_info))
    return;
}
