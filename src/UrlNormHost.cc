...
void ScanHostname(const CHAR* spec, const Component& host,
...
void DoHost(const CHAR* spec,
  ...
  if (!success) {
    ...
    NormalizeIPAddress(output->data(),
...
bool NormalizeHost(const char* spec,
                      const Component& host,
                      CanonOutput* output,
                      Component* out_host) {
  CanonHostInfo host_info;
...
void NormalizeHostVerbose(const char* spec,
                             const Component& host,
                             CanonOutput* output,
                             CanonHostInfo *host_info) {
  DoHost<char, unsigned char>(spec, host, output, host_info);
